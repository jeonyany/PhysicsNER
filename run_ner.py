...
flags.DEFINE_string(
    "data_dir", "/content/gdrive/MyDrive/mydata/v2",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", "/content/gdrive/MyDrive/chinese_L-12_H-768_A-12/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name","ner", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "/content/gdrive/MyDrive/chinese_L-12_H-768_A-12/vocab.txt",
...
flags.DEFINE_string(
    "init_checkpoint", "/content/gdrive/MyDrive/chinese_L-12_H-768_A-12/bert_model.ckpt",
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, logits,num_labels,mask):
                ...
                #cm = metrics.get_metrics_ops(label_ids, predictions, num_labels-1, weights=mask)#减一是减去PAD
                cm = metrics.get_metrics_ops(label_ids, predictions, num_labels)
                return {
                    "confusion_matrix":cm
                }
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        with open(output_eval_file,"w") as wf:
            ...
            p,r,f = metrics.get_metrics(result["confusion_matrix"], len(label_list))
            wf.write('********************P = %s*********************' + str(p) + '\n')
            wf.write('********************R = %s*********************' + str(r) + '\n')
            wf.write('********************F = %s*********************' + str(f) + '\n')
            logging.info("***********************************************")
